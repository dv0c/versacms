...
export async function GET(req: NextRequest) {
  try {
    ...
    if (
      !req.headers.has("sparkle-press") &&
      !req.nextUrl.searchParams.has("api")
    )
      return new Response("API key is missing!", { status: 403 });
    ...
    const token = await db.api.findUnique({
      ...
      where: {
        name:
          req.headers.get("sparkle-press")?.split(" ")[0] ||
          req.nextUrl.searchParams.get("api")?.toString(),
      },
    });

    if (!token?.name)
      return new Response("API key is invalid", { status: 403 });

    if (!session && !token?.name)
      return new Response("Unauthorized", { status: 403 });

    if (token.limit === token.requests)
      return new Response("API limit reached", { status: 403 });
